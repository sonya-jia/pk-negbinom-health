...
- **districts:** Information about different geographic districts of Pakistan. This was taken from Kaggle (1), which extracted the data from Open Data Pakistan (2).
  - *doctors*, the number of doctors in a district. Our response variable.
  - *pop*, the size of the pop force.
...
ggplot(pakistan) +
  ...
  geom_label_repel(
    ...
    stat = "sf_coordinates",  # computes centroids for label placement
    size = 3,
    box.padding = 0.3,
    label.size = 0.2,        # border thickness of white box
...
Things to notice: 
  ...
  - Missing data: Dark grey is the default when there is an NA for the number of doctors in a province. We don't have information for number of doctors in a couple of provinces. Not Punjab is dark gray as well as Gilgit Baltistan and Islamabad Capital Territory.
...
An offset is useful because more populous areas naturally have more doctors; thus, we use pop force size as the offset.
...
The p-value is low. For each increase in the poverty index by one unit, the expected rate of doctors per pop population in a district will change by a multiplicative factor of `{r} exp(-1.54)`. This means we expect the number of doctors to decrease by 78.6% for each point increase in poverty index. 

There are no poverty index values with a one point difference since the poverty index appears to occur between 0 and 1. It may be more meaningful to make interpretations for each 0.1 point increase in the poverty index. For example, the expected rate of doctors per pop population in a district will change by a multiplicative factor of `{r} exp(-1.54*0.1)`. This means we expect the number of doctors to decrease by 14.3% for each 0.1 point increase in poverty index.
...
**Question 7: Create a visualization and summary table to verify or reject the following statement: Districts with public hospitals in Pakistan have more similar doctor-to-pop force ratios than those without public hospitals**
```{r}
districts |>
  mutate(
    doctor_to_pop = doctors/pop
  ) |>
ggplot(aes(x = factor(public_hospitals), y = doctor_to_pop)) +
  geom_boxplot()
```



**Question 7: Create a visualization and summary table to verify or reject the following statement: Districts in the same province in Pakistan have more similar doctor-to-pop force ratios than those in different provinces.**
...
ggplot(aes(x = province, y = doctor_to_pop)) +
  geom_boxplot()
```
...
6. To go a bit extra, it would be great to talk a bit about multi-level models as this seems like a great type of application that could be used for these. You might think about how to integrate a few of these questions if you have time. 

Here are some ideas that might be interesting:

Some initial questions for multi-level models: 
What are the primary variables considered? 
What are the level one and level two observational units? 
What variables are measured at each level? 

Some EDA suggestions, some of which you have already implemented in your assignment: 
1. Explore general association between the predictor and response variable
2. Explore whether subjects at a given level of the predictor tend to have similar mean responses
3. Explore whether variation in response differs at different levels of a predictor
There are a two main ways we can visualize these associations:
- A plot of the response versus the predictor for individual observations (meaning treat the observations as independent). 
- Separate plots of responses versus predictors for each of the level two observations, which is known as a lattice plot. 

Based upon these, can you come up with a proposed composite model and write this out. You may want to guide the student through this and help them with it as we have seen it’s a bit challenging. Focus on some simplicity and a basic approach. 


References


(1) https://opendata.com.pk/organization/about/sdpi
