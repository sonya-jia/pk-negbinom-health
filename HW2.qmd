...
#referenced chat gpt to determine how to fix my specification of the plot function here, asked this prompt "how to plot a line in R"
```
...
A poisson random variable counts independent events that happen at a constant average rate. The exponential distribution can represent hte time between events in such a Poisson process.

## Exercise 4
While an exponential random variable will represent the time between events or before an event, a gamma random variable will represent the time for a certain number of events ot occur. Therefore, a gamma random variable will essentially be a sum of exponential random variables. 
...
## Exercise 8
A binomial distribution would be useful in modeling number of matings. Mating as an event can either occur or not occur, representing success or not success. Since each event consists of number of matings, we need to quantify this by a distribution that has support over [0, 1, 2, 3, ...], so binomial is suitable. We expect there to be a probability of success or probability of mating success for older elephants and probability of mating success for younger elephants with our probability parameter potentially differing across elephant ages. 
