...
leases |>
  group_by(internal_industry) |>
  summarise(subProp = mean(sublet_availability_proportion))
```
...
Thresholding the cost of labor vs type of labor vs amount of labor. 
...
*does DF-squared leas
```{r}
leases <- leases |>
  mutate(sqftHundred = leasedSF/1000,
         subProp = sublet_availability_proportion*100)
...
```{r}
mod <- lmerTest::lmer(
  leasedSF ~ internal_industry + (1|date),  
  data = leases
)
...
# Convert fixed effects into a data frame for ggplot
...
ggplot(fixed_effects_df, aes(x = Term, y = Estimate)) +
  ...
  theme_minimal() +
  labs(title = "Fixed Effects from the Mixed Model", 
       x = "Fixed Effect Term", 
       y = "Estimate") +
...
leases |>
  ...
  group_by(market) |>
  summarize(
    averageLeased = mean(leasedSF),
    subletProp = first(sublet_availability_proportion)
  )|>
  arrange(desc(subletProp))
```
